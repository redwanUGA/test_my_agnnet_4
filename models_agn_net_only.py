...
class TGNMemory(nn.Module):
    ...
    def update(self, node_ids, messages, timestamps):
        self.memory.data[node_ids] = messages
...
class MessageModule(nn.Module):
    def __init__(self, memory_dim, edge_feat_dim):
        ...
        self.mlp = nn.Sequential(
            nn.Linear(memory_dim * 2 + edge_feat_dim, memory_dim),
            nn.ReLU(),
            nn.Linear(memory_dim, memory_dim),
        )

    def forward(self, source_memory, target_memory, edge_features):
        x = torch.cat([source_memory, target_memory, edge_features], dim=1)
        return self.mlp(x)
...
class TGN(nn.Module):
    def __init__(self, num_nodes, memory_dim, edge_feat_dim, out_channels, heads=2):
        ...
        self.msg_module = MessageModule(memory_dim, edge_feat_dim)
        self.attn = TransformerConv(memory_dim, out_channels, heads=heads)

    def forward(self, data):
        src, dst = data.edge_index
        source_memory = self.memory(src)
        target_memory = self.memory(dst)
        edge_features = getattr(data, 'edge_attr', torch.zeros(src.shape[0], 1, device=src.device))
        messages = self.msg_module(source_memory, target_memory, edge_features)
        self.memory.update(dst, messages, getattr(data, 'edge_time', None))
        x = self.attn(self.memory.memory, data.edge_index)
        return x
...
class SinusoidalTimeEncoding(nn.Module):
    ...
    def forward(self, t):
        div_term = torch.exp(torch.arange(0, self.time_dim, 2, device=t.device) * (-math.log(10000.0) / self.time_dim))
        pe = torch.zeros(t.size(0), self.time_dim, device=t.device)
...
class TGAT(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, heads=2, num_layers=2, dropout=0.5, time_dim=16):
        ...
        self.layers = nn.ModuleList([
            TransformerConv(in_channels + time_dim, hidden_channels, heads=heads)
        ])
        for _ in range(num_layers - 2):
            self.layers.append(TransformerConv(hidden_channels * heads, hidden_channels, heads=heads))
        self.layers.append(TransformerConv(hidden_channels * heads, out_channels, heads=1, concat=False))
        self.dropout = dropout

    def forward(self, data):
        t = getattr(data, 'edge_time', torch.zeros(data.edge_index.size(1), device=data.edge_index.device))
        time_emb = self.time_enc(t)
        x = torch.cat([data.x, time_emb[:data.x.size(0)]], dim=1)
        for i, conv in enumerate(self.layers):
